...
**Planning Gaps Identified**:
{
    ...
    # **Critical Change**: Always use structured output for GigaChat
    if hasattr(llm, '__class__') and 'GigaChat' in llm.__class__.__name__:
        logger.info("Forcing GigaChat to use structured output for planner.")
    ...
    if use_structured_output:
        if not isinstance(response, Plan):
            logger.error(f"Expected a Plan object, but got {type(response)}. Switching to reporter.")
            return Command(goto="reporter")
        curr_plan = response.model_dump()
        full_response = response.model_dump_json(indent=2)  # Create string representation
    else:
        try:
            full_response = response.content if hasattr(response, "content") else str(response)
            curr_plan = json.loads(repair_json_output(full_response))
        except (json.JSONDecodeError, AttributeError) as e:
            logger.warning(f"Failed to decode planner response: {e}. Switching to reporter.")
            return Command(goto="reporter")
...
def reporter_node(state: State, config: RunnableConfig):
    ...
    invoke_messages.append(
        HumanMessage(
            content="ВАЖНО: Структурируй свой отчет в соответствии с форматом в промпте. Не забудь включить:\n\n1. Ключевые моменты - Маркированный список самых важных выводов\n2. Обзор - Краткое введение в тему\n3. Детальный анализ - Организованный в логические разделы\n4. Примечание к исследованию (опционально) - Для более всесторонних отчетов\n5. Ключевые источники - Перечисли все ссылки в конце\n\nДля цитирования НЕ включай встроенные цитаты в текст. Вместо этого размести все цитаты в разделе 'Ключевые источники' в конце, используя формат: `- [Название источника](URL)`. Включи пустую строку между каждой цитатой для лучшей читаемости.\n\nПРИОРИТИЗИРУЙ ИСПОЛЬЗОВАНИЕ ТАБЛИЦ MARKDOWN для представления и сравнения данных. Используй таблицы при представлении сравнительных данных, статистики, функций или опций. Структурируй таблицы с четкими заголовками и выровненными столбцами. Пример формата таблицы:\n\n| Функция | Описание | Плюсы | Минусы |\n|---------|----------|-------|--------|\n| Функция 1 | Описание 1 | Плюсы 1 | Минусы 1 |\n| Функция 2 | Описание 2 | Плюсы 2 | Минусы 2 |",
            name="system",
        )
    )
    ...
    if reflection_agent is not None:
        try:
            ...
            while current_iteration < max_iterations:
                ...
                logger.debug(f"Follow-up queries from reflection: {reflection_result.follow_up_queries}")
                ...
                if isinstance(reflection_result.follow_up_queries, list):
                    queries_to_execute = [
                        str(q) for q in reflection_result.follow_up_queries if isinstance(q, (str, dict)) and str(q).strip()
                    ][:max_queries_per_iteration]
                else:
                    queries_to_execute = []

                logger.info(
                    f"Executing {len(queries_to_execute)} follow-up queries (limited from {len(reflection_result.follow_up_queries) if isinstance(reflection_result.follow_up_queries, list) else 0} total)..."
                )
                ...
                if "iteration_results" in locals() and iteration_results:
                    try:
                        ...
                        logger.info(f"Data for merger: iteration_results={iteration_results}, all_research_findings={all_research_findings}")
