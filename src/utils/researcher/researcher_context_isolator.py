...
class ResearcherContextIsolator:
    ...
    def __init__(self, config: Optional[ResearcherContextConfig] = None):
        ...
        self.context_manager = ExecutionContextManager(context_config)
        logger.info(
            f"ResearcherContextIsolator initialized with isolation level: {self.config.isolation_level}"
        )
    ...
    def prepare_isolated_context(
        ...
        logger.debug(
            f"Preparing isolated context for {agent_name} with {len(completed_steps)} completed steps"
        )
        ...
        optimized_steps, context_info = (
            self.context_manager.prepare_context_for_execution(
                filtered_steps, current_step, agent_name
            )
        )

        # Apply researcher-specific context enhancement
        enhanced_context_info = self._enhance_researcher_context(
            context_info, current_step
        )

        logger.debug(
            f"Context isolation completed: {len(filtered_steps)} -> {len(optimized_steps)} steps"
        )
    ...
    def manage_isolated_observations(
        ...
        optimized_observations = self.context_manager.manage_observations_advanced(
            all_observations, optimization_level=optimization_level
        )
    ...
    def _prepare_isolated_context_from_state(
        self, state: State, agent_name: str = "researcher"
    ) -> State:
        ...
        optimized_state = {
            ...
            "enable_background_investigation": state.get(
        ...
        if optimized_state.get("observations"):
            optimization_level = self._get_optimization_level()
            optimized_observations = self.context_manager.manage_observations_advanced(
                optimized_state["observations"], optimization_level=optimization_level
            )
        ...
        if "messages" in optimized_state and optimized_state["messages"]:
            ...
            if len(optimized_state["messages"]) > max_messages:
                optimized_state["messages"] = optimized_state["messages"][
                    -max_messages:
                ]

        logger.debug(
            f"State isolation completed: {len(step_history)} -> {len(filtered_steps)} steps"
        )
        return optimized_state

    def _apply_isolation_filtering(
        ...
        if self.config.isolation_level == "minimal":
            ...
            return (
                completed_steps[-self.config.max_context_steps * 2 :]
                if completed_steps
                else []
            )
            ...
            return (
                completed_steps[-self.config.max_context_steps :]
                if completed_steps
                else []
            )

    def _enhance_researcher_context(
        self, context_info: str, current_step: Dict[str, str]
    ) -> str:
    ...
    def _truncate_step_content(
        ...
        for step in steps:
            ...
            if len(execution_res) > self.config.max_step_content_length:
                ...
                truncated_content = (
                    execution_res[: self.config.max_step_content_length] + "...[截断]"
                )
    ...
    def generate_refined_output(self) -> str:
        ...
        if hasattr(self, "isolated_analysis_steps") and self.isolated_analysis_steps:
            analysis_summary = (
                f"Analysis completed: {len(self.isolated_analysis_steps)} steps"
            )
            key_findings.append(analysis_summary)

        # Extract key thoughts
        if (
            hasattr(self, "isolated_intermediate_thoughts")
            and self.isolated_intermediate_thoughts
        ):
    ...
    def estimate_context_size(self) -> int:
        ...
        if hasattr(self, "isolated_search_results"):
            total_size += sum(
                len(str(result)) for result in self.isolated_search_results
            )
        ...
        if hasattr(self, "isolated_intermediate_thoughts"):
            total_size += sum(
                len(thought) for thought in self.isolated_intermediate_thoughts
            )
